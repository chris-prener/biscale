...
Note that an `NA` value is passed to `dig_lab` since `pctWhite_bin` has already been created as a factor. If you are using `classInt::classIntervals()` to create your factor, use that functions `dig_lab` argument instead to prepare your labels or breaks to the desired number of decimal places.
