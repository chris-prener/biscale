...
## What's New
`biscale` v0.1.2.9000 is underway! This will be a maintence release that includes typo corrections. It does add the ability to pass objects to `bi_legend()`'s x and y axis label arguments, which may be useful to some users.
...
With our legend drawn, we can then combine the legend and the map with `cowplot`. The values needed for this stage will be subject to experimentation depending on the shape of the map itself. *As of June 21, this workflow has been tested with the new [release candidate for `cowplot`](https://github.com/wilkelab/cowplot) and works as expected.*
