...
The `dim` argument is used to control the extent of the legend - do you want to produce a two-by-two map (`dim = 2`), a three-by-three map (`dim = 3`), or a four-by-four map (`dim = 4`)? 
...
Note that it is also possible to apply custom color palettes as well using the `bi_pal_manual()` function
